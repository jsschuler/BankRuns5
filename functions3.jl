...
function agtSimulate(agt::Agent)
    ...
    global neighborDepth
    global agtList
    # if the neighbor depth is 0, agents can see the entire population 
    if neighborDepth==0
        myNeighbors=agtList

    else
        myNeighbors=Agent[agt]
        for dpth in 1:neighborDepth
            for agt in myNeighbors
                for nbh in neighbors(agt)
                    push!(myNeighbors,nbh)
                end
            end
        end
        myNeighbors=collect(setdiff(Set(myNeighbors),Set([agt])))
    end
